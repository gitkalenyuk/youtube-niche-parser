...
**25 червня 2026 — зрозумілі причини «0 результатів» + важливий фікс .dmg:**

- 🍎 **ВАЖЛИВО: виправлено SSL у `.dmg` з Releases.** Виправлення сертифікатів (`certifi`) раніше додали лише в `build_mac.sh`, але **не** в хмарну збірку GitHub Actions — тож опублікований `.dmg` усе одно міг падати на macOS з нульовим результатом. Тепер `certifi` вкладається й у `.dmg` (`--collect-data certifi`). Це найімовірніша причина «0 результатів» у тих, хто качав `.dmg`.
- 🔍 **Тепер видно, ЧОМУ 0 результатів.** Раніше програма показувала мовчазний 0. Тепер у «Живому лозі» для кожного запиту чітко пишеться причина: **«пошук дав 0»** (завузив дату/тип/тривалість) vs **«фільтри лишили 0»** (мін.перегляди / макс.підписники надто жорсткі), а в кінці — підсумок. Найчастіша причина масових нулів — фільтр **ДАТИ** на вічнозелених темах (свіжих відео цього тижня просто нема).
- 🔁 **Додано retry і точніше виявлення блоку.** Запит повторюється до 3 разів при тимчасовій помилці/429, а помилка «consent» більше не плутається з блоком (хибне спрацювання прибрано).

**14 червня 2026 — виправлення стабільності:**

- 💾 **Виправлено експорт CSV / TXT** (Windows і Mac). Раніше в режимі вікна-додатка (Chrome / Edge `--app`) збереження могло мовчки не спрацьовувати. Тепер файл зберігає сам застосунок у папку **Завантаження** (`~/Downloads`) і показує точний шлях до нього. CSV пишеться з BOM, щоб Excel коректно відкривав кирилицю. Є й запасний шлях завантаження, якщо щось піде не так.
...
- 📊 **Виправлено сортування по колонці V/S.** Клік по заголовку **V/S** не працював (значення обчислюване, а не збережене поле). Тепер сортування за V/S працює коректно — і за спаданням, і за зростанням.

**Що саме змінено в коді:**

- `app.py` — доданий ендпоінт `/api/save`, який зберігає експорт на диск на боці сервера.
- `ui.html` — кнопки «Зберегти CSV / TXT» тепер шлють дані на `/api/save` і показують результат; виправлено запасне blob-завантаження; виправлено сортування колонки **V/S** (рахується через `gemRatio` у компараторі).
- `scraper.py` — SSL-контекст будується з CA-бандлу `certifi` (з безпечним відкатом).
- `build_mac.sh` — `certifi` встановлюється й вкладається в застосунок під час збірки.

**Оновлені завантаження:** усі три файли в Releases перезібрано з виправленнями — `YouTube-Niche-Parser-Windows.zip`, `YouTube-Niche-Parser.dmg` та `YouTube-Niche-Parser-Mac.zip`.
